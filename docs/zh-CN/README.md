...
NineA 目前是用于本地评估的源码 Alpha，需要 Unix domain socket。当前不提供
provider sandbox、HTTP MCP transport、流式执行、多轮续接、预编译安装包或稳定兼容
承诺。MCP server 和 executable adapter 都是受信本地进程，以 daemon 用户的操作系统
权限运行。
